...
<div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <h2 class="text-2xl font-bold mb-6">Novo Profissional</h2>

            <form method="POST" action="{{ route('profissionais.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="space-y-4">
                    <!-- Foto/Avatar -->
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="percentual_comissao" value="{{ old('percentual_comissao', 50) }}" min="0" max="100" step="0.01" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('percentual_comissao')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
